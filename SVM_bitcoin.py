...
def trainAndtest(money, prevDays, short, long):
    ...
    imp_features = train_data[['Day Diff', 'SIA', 'Volatility']].values
    ...
    while i < len(test_data):
        ...
        if money<test_data['Close**'][i]:
            messagebox.showwarning("Not enough funds", "You do not have enough money.")
            print("Not enough funds. Terminating the algorithm.")
            break
    ...
    if finalProffitLoss>0:
        messagebox.showinfo('Conclusion', 'Accuracy: '+str(round(final_accuracy,2))+'%\n\nFinal wallet balance: '+str(round(money, 2))+"$"+
                            "\n\nProfit: "+str(round(finalProffitLoss, 2))+"$")
    else:
